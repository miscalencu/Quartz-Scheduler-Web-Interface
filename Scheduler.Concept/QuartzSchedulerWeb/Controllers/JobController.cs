...
namespace QuartzSchedulerWeb.Controllers
{
    public class JobController : Controller
    {
		...
		public ActionResult SyncJobs(int ID = 0)
		{
			try
			{
				...
				IScheduler sched = QuartzHelper.GetScheduler();
				...
				foreach (string group in jobGroups)
				{
					...
					foreach (var jobKey in jobKeys)
					{
						...
						{
							foreach (ITrigger triger in sched.GetTriggersOfJob(jobKey))
							{
								// remove schedulers
								// sched.UnscheduleJob(triger.Key);
							}

							sched.DeleteJob(jobKey); // remove it from the service
						}
					}
				}
